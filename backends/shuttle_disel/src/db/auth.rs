...
impl Handler<GenerateAuth> for DbExecutor {
    type Result = MessageResult<GenerateAuth>;

    fn handle(&mut self, msg: GenerateAuth, _: &mut Self::Context) -> Self::Result {
        use crate::schema::users::dsl::*;

        let claims = msg.token.decode_jwt().unwrap().claims;
        let mut conn = self.0.get().unwrap();

        match users.find(claims.id).first::<User>(&mut conn) {
            Err(_) => MessageResult(Err(AppError::Unauthorized("Invalid Token"))),
            Ok(user) => MessageResult(Ok(Auth {
                user,
                token: msg.token,
            })),
        }
